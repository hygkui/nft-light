...
/*
 * @dev Provides information about the current execution context, including the 提供信息关于当前执行环境的上下文
 * sender of the transaction and its data. While these are generally available 交易的发送方和数据，主要包含在msg.sender 和
...
contract YieldToken is ERC20("Banana", "BANANA") {
	...
	uint256 constant public BASE_RATE = 10 ether; // 保证金？ 基本利率？
	uint256 constant public INITIAL_ISSUANCE = 300 ether; // 初次发行
	// Tue Mar 18 2031 17:46:47 GMT+0000
	uint256 constant public END = 1931622407; // 公开时间

	mapping(address => uint256) public rewards; // 奖励的地址列表
	mapping(address => uint256) public lastUpdate; // 最近更新的地址列表
	...
	function updateRewardOnMint(address _user, uint256 _amount) external {
		...
		uint256 time = min(block.timestamp, END); // 区块链时间戳和end时间比较
...
contract Kongz is ERC721Namable, Ownable {
	...
	uint256 constant public BREED_PRICE = 600 ether; // 繁殖 再生
	...
	constructor(string memory _name, string memory _symbol, string[] memory _names, uint256[] memory _ids) public ERC721Namable(_name, _symbol, _names, _ids) {
		...
		_mint(msg.sender, 1001); // 相当于预先挖了，团队保留了。
